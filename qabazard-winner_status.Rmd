...
Since not all rounds are comparable for the aforementioned reasons, I run separate models for the cross-section that cannot be pooled (Rounds 1, 2, and 4) and I pooled model that includes respondent from the third, fifth, and sixth round.   
...
pooled$mean_pol_turnover[pooled$year==2006] <- 0.584
pooled$mean_pol_turnover[pooled$year==2013] <- 0.895
pooled$mean_pol_turnover[pooled$year==2015] <- 1.001

pooled$Diff_pol_turnover <- pooled$pol_turnover - pooled$mean_pol_turnover

pooled$mean_years_since_election[pooled$year==2006] <- 0.584
pooled$mean_years_since_election[pooled$year==2013] <- 0.895
pooled$mean_years_since_election[pooled$year==2015] <- 1.001

pooled$Diff_years_since_election <- pooled$years_since_election - pooled$mean_years_since_election


pooled$country_wave <- (pooled$state * pooled$year)
...
* *Years from Election:* ranges from 0 to 9.

* *Year:* Last year surveys were conducted in that particular round of AB surveys.

* *Number of Political Turnovers:* ranges from 0 to 5.
...
Figure_A4 <- dwplot(list(A4Model3_W3W5W6, A4Model3_W3W5W62),
       vline = geom_vline(xintercept = 0, colour = "grey60", linetype = 2)) %>% # plot line at zero _behind_ coefs
    relabel_predictors(c(winner = "Winner",                       
                         independent = "Independent", 
                         free_fair = "Electoral Integrity", 
